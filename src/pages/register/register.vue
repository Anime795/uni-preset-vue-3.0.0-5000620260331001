  ...
  <view class="page" :style="{ paddingTop: statusBarHeight + 'px' }">
    ...
    <view class="inner">
      ...
      <view class="brand">
        ...
        <text class="subtitle">使用账号密码完成注册</text>
      </view>
      ...
      <view class="card">
        ...
        <view class="field">
          ...
          <view class="input-row">
            <text class="input-icon" aria-hidden="true">✉️</text>
            <input
              id="register-email"
              class="input"
              type="text"
        ...
        <view class="row-actions row-actions--left">
          <text class="hint-text">密码将用于登录，请妥善保管</text>
        </view>
        ...
        >
          ...
          <text class="btn-primary-text">{{ submitting ? '提交中…' : '注 册' }}</text>
        </button>
      </view>

      <view class="footer footer--action" @tap.stop="goToLogin" @click.stop="goToLogin">
        <text class="footer-text">已有账号 ·</text>
        <text class="footer-link">登录</text>
      </view>
    </view>
  </view>
</template>
...
function goToLogin() {
  if (loginNavLock.value) return
  loginNavLock.value = true
  uni.redirectTo({
    url: '/pages/login/login',
    fail(err) {
      uni.showToast({ title: '返回登录失败', icon: 'none' })
      console.error('[goToLogin]', err)
    },
    complete() {
      loginNavLock.value = false
    },
  })
}
